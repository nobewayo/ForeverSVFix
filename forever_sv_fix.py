...
ELLESMERE_COMPAT = "ForeverSVFixEllesmereUI.lua"  # legacy v1.0.3-and-earlier artifact
ELLESMERE_ADDON = "EllesmereUI"
ELLESMERE_FIX_URL = "https://www.curseforge.com/wow/addons/ellesmereui-forever-fix"
...
def character_stores(account: Path) -> list[CharacterStore]:
    ...
    for saved_dir in account.rglob("SavedVariables"):
        ...
        if len(parts) < 3 or parts[-1] != "SavedVariables":
            continue
        # The current beta usually uses <realm>/<first-last>/SavedVariables,
        # but tolerate a character name being split across extra path components.
        # Everything between the realm folder and SavedVariables belongs to the
        # character identity used for matching.
        realm_folder = parts[0]
        character_folder = "/".join(parts[1:-1])
        result.append(CharacterStore(realm_folder, character_folder, saved_dir))
...
def windows_reparse_point(path: Path) -> bool:
    # Detect Windows junctions/reparse points without following them.
    try:
...
def remove_link(path: Path) -> None:
    ...
    if platform.system() == "Windows":
        ...
        # Addon updaters can replace a ForeverSVFix junction with a real
        # directory containing copied files. The path is ForeverSVFix-owned,
        # so remove that stale directory and recreate the link cleanly.
        if path.is_dir():
            try:
                shutil.rmtree(path)
            except OSError as exc:
                raise FixError(
                    f"Could not remove stale ForeverSVFix directory {path}: {exc}"
                ) from exc
            return
...
def generate_char_bootstrap(
    ...
    lines = [
        ...
        "  end",
        "",
        "  local fullPlayer, realm",
        '  if UnitFullName then fullPlayer, realm = UnitFullName("player") end',
        "  local shortPlayer",
        '  if UnitName then shortPlayer = UnitName("player") end',
        '  if not fullPlayer or fullPlayer == "" then fullPlayer = shortPlayer end',
        ...
        "",
        '  local firstPlayer = fullPlayer and fullPlayer:match("^%S+") or nil',
        "  local np, ns, nf, nr = norm(fullPlayer), norm(shortPlayer), norm(firstPlayer), norm(realm)",
        "  local stores = {",
    ]
    ...
    lines += [
        "  }",
        "",
        "  local exact, short, fallback = {}, {}, {}",
        "  for _, item in ipairs(stores) do",
        "    local nc = norm(item.character)",
        "    if np ~= '' and (nc == np or (nr ~= '' and nc == (np .. nr))) then",
        "      table.insert(exact, item)",
        "    elseif (ns ~= '' and nc == ns) or (nf ~= '' and nc == nf) then",
        "      table.insert(short, item)",
        "    elseif np ~= '' and nc ~= '' then",
        "      if nc:find(np, 1, true) == 1 or np:find(nc, 1, true) == 1 then",
        ...
        "  elseif #exact == 0 and #short == 1 then chosen = short[1]",
        "  elseif #exact == 0 and #short == 0 and #fallback == 1 then chosen = fallback[1] end",
        "",
        "  if chosen then",
        "    local ok, reason",
        ...
        '      .. addonKey .. " (" .. tostring(fullPlayer or "?") .. "-" .. tostring(realm or "?") .. ")")',
        "  end",
        "end",
        "",
    ]
...
def patch_toc(
    ...
    # ELLESMERE_COMPAT remains recognized here only so v1.0.4 can migrate
    # installations created by v1.0.3 and earlier.
    cleaned = []
    for line in lines:
    ...
    restore_before_scripts = loads_saved_variables_first(lines)
...
def _path_within(candidate: Path, root: Path) -> bool:
    # Resolve without requiring the candidate to exist.
    try:
...
def legacy_ellesmere_artifacts(wow: Path) -> dict:
    # Find v1.0.3-and-earlier built-in EllesmereUI compatibility artifacts.
    addons_dir = wow / "Interface" / "AddOns"
    eui_dir = addons_dir / ELLESMERE_ADDON
    runtime_files: set[Path] = set()
    tocs: set[Path] = set()

    primary = eui_dir / ELLESMERE_COMPAT
    if primary.is_file():
        runtime_files.add(primary)

    if eui_dir.is_dir():
        for toc in eui_dir.glob("*.toc"):
            try:
                if ELLESMERE_COMPAT in read_text(toc):
                    tocs.add(toc)
    ...
    if state:
        value = state.get("ellesmere_compat_files", [])
        if isinstance(value, list):
            legacy_state_entries = [v for v in value if isinstance(v, str)]
            for raw in legacy_state_entries:
                candidate = Path(raw)
                if (
                    candidate.name == ELLESMERE_COMPAT
                    and _path_within(candidate, addons_dir)
                    and candidate.is_file()
                ):
                    runtime_files.add(candidate)

    return {
        "runtime_files": sorted(runtime_files, key=str),
        "tocs": sorted(tocs, key=str),
        "state_has_legacy": bool(legacy_state_entries),
    }


def legacy_ellesmere_present(legacy: dict) -> bool:
    return bool(
        legacy.get("runtime_files")
        or legacy.get("tocs")
        or legacy.get("state_has_legacy")
    )


def backup_legacy_ellesmere(wow: Path, backup: Path, legacy: dict) -> None:
    root = backup / "legacy-ellesmere"

    for toc in legacy.get("tocs", []):
        dest = root / "tocs" / toc.parent.name / toc.name
        dest.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(toc, dest)

    for runtime in legacy.get("runtime_files", []):
        dest = root / "runtime" / runtime.parent.name / runtime.name
        dest.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(runtime, dest)

    state_file = state_dir(wow) / STATE_FILE_NAME
    if state_file.is_file():
        root.mkdir(parents=True, exist_ok=True)
        shutil.copy2(state_file, root / STATE_FILE_NAME)


def cleanup_legacy_ellesmere(wow: Path, legacy: dict) -> None:
    addons_dir = wow / "Interface" / "AddOns"

    for runtime in legacy.get("runtime_files", []):
        if (
            runtime.name == ELLESMERE_COMPAT
            and _path_within(runtime, addons_dir)
            and runtime.is_file()
        ):
            runtime.unlink()

    # patch_toc/reconcile normally removes the old TOC line. Sweep it
    # defensively for migration-only installs.
    for toc in legacy.get("tocs", []):
        if not toc.is_file() or not _path_within(toc, addons_dir):
            continue
        original = read_text(toc)
        lines = [line for line in original.splitlines() if line.strip() != ELLESMERE_COMPAT]
        updated = "\n".join(lines) + ("\n" if original.endswith(("\n", "\r")) else "")
        if updated != original:
            write_text(toc, updated)
...
def install(
    ...
    allow_legacy_ellesmere_cleanup: bool = False,
) -> int:
    account = choose_account(wow, account_name)
    ...
    legacy = legacy_ellesmere_artifacts(wow)
    has_legacy = legacy_ellesmere_present(legacy)

    if not targets and not has_legacy:
        raise FixError("No compatible existing SavedVariables files were found.")

    if has_legacy and not allow_legacy_ellesmere_cleanup:
        raise FixError(
            "Legacy ForeverSVFix EllesmereUI compatibility was detected. "
            "Run interactive Apply / Refresh and approve the migration, or use "
            "--migrate-legacy-ellesmere with install/repair."
        )

    backup = next_backup_dir(wow)
    if has_legacy:
        backup_legacy_ellesmere(wow, backup, legacy)

    target_tocs = {info.path for info, _, _ in targets}
    target_folders = {info.path.parent for info, _, _ in targets}

    stale_cleaned = reconcile_old_patches(addons_dir, target_tocs, target_folders)
    ...
    try:
        ...
        if has_legacy:
            cleanup_legacy_ellesmere(wow, legacy)

    except Exception:
        print(f"ERROR during install. Safety backup is at: {backup}", file=sys.stderr)
        raise
    ...
    if has_legacy:
        print("Legacy EllesmereUI shim: migrated/removed")
        print(f"EllesmereUI compatibility now lives in: {ELLESMERE_FIX_URL}")
...
def doctor(
    ...
    if has_legacy:
        warnings.append(
            "Legacy ForeverSVFix EllesmereUI compatibility detected. "
            "Run Apply / Refresh and approve migration to EllesmereUI Forever Fix."
        )
    ...
    if show_status:
        ...
        if state:
            ...
            if has_legacy:
                print("Legacy EllesmereUI:   MIGRATION NEEDED")
...
def interactive_apply_refresh(
    wow: Path,
    account: str | None,
) -> int:
    legacy = legacy_ellesmere_artifacts(wow)
    if not legacy_ellesmere_present(legacy):
        return install(wow, account)

    print()
    print("=" * 54)
    print("ELLESMEREUI MIGRATION")
    print()
    print("This installation still contains the EllesmereUI compatibility shim")
    print("created by ForeverSVFix 1.0.3 or earlier.")
    print()
    print("EllesmereUI-specific fixes have moved to the standalone")
    print("'EllesmereUI Forever Fix' addon:")
    print(ELLESMERE_FIX_URL)
    print()
    print("ForeverSVFix is still required for SavedVariables persistence.")
    print("The standalone addon is NOT installed automatically.")
    print()
    answer = input(
        "Remove the old ForeverSVFix EllesmereUI shim and continue? [y/N]: "
    ).strip().lower()
    if answer not in ("y", "yes"):
        print()
        print("Apply / Refresh cancelled. No changes were made.")
        return 1
...
def interactive_menu() -> int:
    ...
    while True:
        ...
        if choice == "1":
            run_menu_action(interactive_apply_refresh, wow, account)
        elif choice == "2":
    ...
    for name in ("scan", "install", "repair", "doctor", "status", "uninstall", "check-update"):
        ...
        if name in ("install", "repair"):
            command_parser.add_argument(
                "--migrate-legacy-ellesmere",
                action="store_true",
                help=(
                    "Allow noninteractive cleanup of the legacy built-in "
                    "EllesmereUI compatibility shim."
                ),
            )
    return p
    ...
    try:
        ...
        if args.command in ("install", "repair"):
            return install(
                wow,
                account,
                allow_legacy_ellesmere_cleanup=getattr(
                    args,
                    "migrate_legacy_ellesmere",
                    False,
                ),
            )
