...
def patch_toc(
    ...
    # Remove any previous ForeverSVFix v2/v3 metadata and owned file entries.
    cleaned = []
    for line in lines:
...
def status(wow: Path) -> int:
    ...
    print(f"Patched TOCs:       {len(state.get('patched_tocs', []))}")
    print(f"Account links:      {len(state.get('account_links', []))}")
    print(f"Character helpers:  {len(state.get('pc_helpers', []))}")
